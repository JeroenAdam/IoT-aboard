...
My current project is developing a solution based on such a Signal K server combined with my bright LED display for the purpose of safety when navigating long routes. The server is hosted on a Raspberry Pi running OpenPlotter (=Debian Linux customized for marine use).
